/**
 * Build carousel slide-2 safari collage from wildlife photos.
 * Run: node scripts/build-hero2-safari-collage.mjs
 */
...
const outDir = path.join(root, 'scripts/.tmp-hero2-collage');
...
$filters = @{
  leopardJeeps='*hero-5-safari-febbf88f*';
  elephant='*1-b0244bce*';
  treeLeopards='*rapszppomvhjac9sd3sb-11e76a72*'
}
...
let html = await readFile(path.join(__dirname, 'hero2-safari-collage-template.html'), 'utf8');
...
await writeFile(path.join(carouselDir, 'hero-5-safari.jpg'), Buffer.from(await encodeJpeg(full, { quality: 86 })));
...
for (const { w, webpQ, jpegQ } of widths) {
  ...
  await writeFile(path.join(carouselDir, `hero-5-safari-${w}.webp`), Buffer.from(webp));
  await writeFile(path.join(carouselDir, `hero-5-safari-${w}.jpg`), Buffer.from(jpg));
  console.log(`hero-5-safari-${w}.webp ${(webp.byteLength / 1024).toFixed(1)} KiB`);
}

await copyFile(path.join(carouselDir, 'hero-5-safari-1920.webp'), path.join(carouselDir, 'hero-5-safari.webp'));
await writeFile(path.join(outDir, 'hero-5-safari-collage-preview.jpg'), Buffer.from(jpgBuf));
await copyFile(
  path.join(outDir, 'hero-5-safari-collage-preview.jpg'),
  path.join(carouselDir, 'hero-5-safari-collage-preview.jpg'),
);

console.log('Updated hero-5-safari carousel assets (slide 2 collage).');
